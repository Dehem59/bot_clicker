...
class ExecutionAdsView(View):

    http_method_names = ["get", "post", "patch"]

    def get(self, request, *args, **kwargs):
        user_agents = UserAgent.objects.all()
        return render(request, "gui/exec_ads.html", {"nb_proxy": len(Proxy.objects.all()), "user_agents": user_agents})
    ...
    def post(self, request, *args, **kwargs):
        ...
        for proxy_obj in Proxy.objects.filter(est_actif=True):
            ...
            try:
                tmp_bot = BotClickerV1(proxy=proxy, query=params["query"], domain=params["domaine"], google_ads=True,
